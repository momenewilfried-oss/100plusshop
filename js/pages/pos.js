async function renderPOS(el) {
  ...
  function totalPanier() {
    return panier.reduce(function (s, l) {
      return s + l.prixUnitaire * l.quantite - (l.remise || 0);
    }, 0);
  }
  ...
  function clientOptionsHtml(q) {
    ...
    var opts = clients
      .filter(function (c) {
        var txt = ((c.prenom || '') + ' ' + (c.nom || '')).toLowerCase();
  ...
  function render() {
    ...
    var locked = paiementEnCours;

    el.innerHTML =
      '<div class="grid-2">' +
      '<div class="panel"><div class="panel-title">Catalogue</div>' +
      ...
      (produits.length === 0
        ...
        : produits
            .map(function (p) {
              var id = p.id_produit || p.id;
              var search = ((p.nom || '') + ' ' + (p.reference || '')).toLowerCase();
              return (
            ...
            .join('')) +
      '</div></div>' +
      '<div class="panel"><div class="panel-title">Panier</div>' +
      (panier.length === 0
          ...
          panier
            .map(function (l, i) {
              return (
                ...
                '</span></td><td><input type="number" min="1" value="' +
                l.quantite +
                '" data-qte="' +
                i +
                ...
                '</strong></td><td><button class="btn btn-sm" data-rm="' +
                i +
                '"' +
                (locked ? ' disabled' : '') +
                '>✕</button></td></tr>'
            ...
            .join('') +
          '</tbody></table>' +
          '<div style="margin-top:16px;text-align:right;"><div style="font-size:13px;color:var(--muted);">Total</div>' +
          '<div style="font-size:24px;font-weight:700;color:var(--primary);">' +
          formatMontant(total) +
          '</div></div>') +
      '<div class="form-grid" style="margin-top:16px;"><div>' +
      '<label>Client enregistré (optionnel)</label>' +
      '<input type="search" id="posClientSearch" placeholder="Rechercher un client…" style="width:100%;margin-bottom:6px;height:32px;padding:0 8px;border:1px solid var(--border);border-radius:6px;"' +
      (locked ? ' disabled' : '') +
      ' />' +
      '<select id="posClient"' +
      (locked ? ' disabled' : '') +
      ...
      clientOptionsHtml() +
      '</select>' +
      '<label style="margin-top:8px;display:block">Ou nom libre</label>' +
      '<input type="text" id="posClientLibre" placeholder="Nom du client (non enregistré)" maxlength="200" style="width:100%;margin-top:4px;height:32px;padding:0 8px;border:1px solid var(--border);border-radius:6px;"' +
      (locked ? ' disabled' : '') +
      ' /></div><div><label>Paiement</label>' +
      '<select id="posPay"' +
      (locked ? ' disabled' : '') +
      '>' +
      ...
      '</button>' +
      (locked
        ? '<div style="margin-top:8px;font-size:12px;color:var(--muted);text-align:center;">Vente et facture en cours…</div>'
        : '') +
      '</div></div>' +
      ...
      '<button class="btn" id="posModalClose" style="margin-top:12px;width:100%;">Fermer</button></div></div>';
    ...
    el.querySelectorAll('[data-add-prod]').forEach(function (btn) {
      btn.addEventListener('click', async function () {
        ...
        var idProduit = Number(btn.dataset.addProd);
        var nom = btn.closest('[data-nom]')?.dataset.nom || 'Produit';
        var modal = document.getElementById('posModal');
        var body = document.getElementById('posModalBody');
        ...
        if (!variantes.length) {
          body.innerHTML = '<div class="empty">Aucune variante</div>';
          return;
        }
        body.innerHTML = variantes
          .map(function (v) {
            ...
            return (
              '<div class="alert-item"><div><div class="name">' +
              escapeHtml(v.taille || '-') +
              ' / ' +
              escapeHtml(v.couleur || '-') +
              '</div><div class="meta">Stock: ' +
              stock +
              ' · ' +
        ...
        body.querySelectorAll('[data-pick]').forEach(function (b) {
          b.addEventListener('click', function () {
            if (paiementEnCours) return;
            var idVariante = Number(b.dataset.pick);
            var existing = panier.find(function (x) {
    ...
    document.getElementById('posClientSearch')?.addEventListener('input', function (e) {
      ...
      var exact = clients.find(function (c) {
    ...
    el.querySelectorAll('[data-qte]').forEach(function (input) {
      input.addEventListener('change', function () {
        if (paiementEnCours) return;
        var i = Number(input.dataset.qte);
    ...
    document.getElementById('btnPayer')?.addEventListener('click', async function () {
      ...
      try {
        var idClient = document.getElementById('posClient')?.value;
        var clientLibre = (document.getElementById('posClientLibre')?.value || '').trim();

        var vente = await VentesAPI.create({
        ...
        var idVente = Number(vente.id_vente || vente.id);

        try {
          var facture = vente.facture || null;
          if (!facture && idVente) {
          ...
          console.warn(eFact);
          alert('Vente #' + idVente + ' OK, facture : ' + (eFact.message || eFact));
        }
